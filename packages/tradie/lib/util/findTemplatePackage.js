...
class NoTemplateError extends Error {
  constructor() {
    super(`tradie: No template found in \`devDependencies\` in \`package.json\`. Please install a template e.g. \`npm install --save tradie-template-default\`.`);
...
class MultipleTemplatesError extends Error {
  constructor() {
    super(`tradie: More than one template found in \`devDependencies\` in \`package.json\`. Please uninstall a template e.g. \`npm uninstall tradie-template-default\`.`);
    this.name = this.constructor.name;
  }
}
...
module.exports = (dir) => findAndReadPackageJson(dir, {cache: PACKAGE_CACHE, transform: json => json.devDependencies}) //TODO: move cache to a higher level
  .then(devDependencies => {
    ...
    const templates = Object.keys(devDependencies).filter(name => TEMPLATE_NAME_REGEXP.test(name));
