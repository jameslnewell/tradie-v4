...
Template.find = function(name) {
  ...
  return import(path.join(
    ...
    if (!templateName) {
      ...
      const templateNames = projectDependencies.filter(isTemplateName);

      if (templateNames.length === 0) {
        return Promise.reject(
          // new Error( //FIXME: https://github.com/facebook/jest/issues/3699
          `tradie: No template found in \`devDependencies\` in \`package.json\`. Please install a template e.g. \`npm install --save @tradie/default-template\`.`
          // )
        );
      }
      ...
      if (templateNames.length > 1) {
        return Promise.reject(
          // new Error( //FIXME: https://github.com/facebook/jest/issues/3699
          `tradie: More than one template found in \`devDependencies\` in \`package.json\`. Please uninstall extraneous templates e.g. \`npm uninstall @tradie/default-template\`.`
          // )
        );
      }
